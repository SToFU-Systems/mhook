...
/**
 * @brief Checks whether a character is a valid hexadecimal digit.
 * @param[in] ch Character to test.
 * @return TRUE when ch is '0'-'9', 'A'-'F', or 'a'-'f'; FALSE otherwise.
 */
...
/**
 * @brief Parses a hex-digit string into a newly allocated byte buffer.
 * @param[in] Input Hex string to parse; optionally quoted, and formatted as space-separated byte pairs ("XX XX"), backslash-escaped bytes introduced by \x or \0 (e.g. "\xAB"), or a continuous run of hex digit pairs.
 * @param[in] InputLength Number of characters available in Input.
 * @param[out] OutputLength Number of bytes written to the returned buffer, or zero on failure.
 * @return Newly allocated buffer containing the decoded bytes, or NULL when Input is empty, malformed, or the buffer cannot be allocated.
 * @remark The caller must free the returned buffer.
 */
