...
#define APPENDPAD(x)                                                                                                   \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            if (Instruction->StringIndex > x)                                                                          \
                assert(0);                                                                                             \
            while (x != Instruction->StringIndex)                                                                      \
                APPENDB(' ');                                                                                          \
        }                                                                                                              \
...
ARCHITECTURE_FORMAT_FUNCTIONS X86 = {X86_InitInstruction, NULL, X86_GetInstruction, X86_FindFunctionByPrologue};
...
#define APPLY_OFFSET(addr)                                                                                             \
    {                                                                                                                  \
        ...
        {                                                                                                              \
        case 8:                                                                                                        \
            addr = ((U64)(addr + Instruction->VirtualAddressDelta));                                                   \
            break;                                                                                                     \
        case 4:                                                                                                        \
            addr = (U64)((U32)(addr + Instruction->VirtualAddressDelta));                                              \
            break;                                                                                                     \
        case 2:                                                                                                        \
            addr = (U64)((U8)(addr + Instruction->VirtualAddressDelta));                                               \
            break;                                                                                                     \
        default:                                                                                                       \
            assert(0);                                                                                                 \
            break;                                                                                                     \
...
/**
 * @brief Resets the X86-specific decode state and sets its default address/operand sizes for the instruction's architecture.
 * @param[in,out] Instruction Instruction whose X86 sub-structure is cleared and initialized; its architecture type (set by the caller) is read to select the sizes.
...
/**
 * @brief Formats a memory operand as "size segment:[base+index*scale+displacement]", appending the resolved target address when the operand is IP-relative.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand Memory operand supplying the length and, for an IP-relative operand, the target address.
 * @param[in] OperandIndex Index of Operand within Instruction, asserted to match the recorded source or destination addressing operand.
 */
...
/**
 * @brief Formats an OPTYPE_a bound operand as "(low, high)" by printing the address twice, once for each half of the range.
 * @param[in,out] Instruction Instruction being formatted; its displacement is temporarily offset by half the operand length to reach the high address, then restored.
 * @param[in,out] Operand Bound operand; its length is temporarily halved to format each half as a plain address, then restored.
 * @param[in] OperandIndex Index of Operand within Instruction, passed through to OutputAddress().
 */
void OutputBounds(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
    X86_INSTRUCTION* X86Instruction = &Instruction->X86;
...
/**
 * @brief Formats a general-purpose operand as a memory address if it is the instruction's recorded addressing operand, otherwise as its register name.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand General-purpose operand supplying either the register to name or the address to format.
 * @param[in] OperandIndex Index of Operand within Instruction, used to test against the recorded source/destination addressing index.
 */
void OutputGeneral(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
    X86_INSTRUCTION* X86Instruction = &Instruction->X86;
...
/**
 * @brief Formats an OPTYPE_dt descriptor-table operand as a memory address.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand Descriptor-table operand to format as an address.
 * @param[in] OperandIndex Index of Operand within Instruction, asserted to match the recorded source or destination addressing operand.
 */
void OutputDescriptor(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Formats a packed-real operand (OPTYPE_ps/OPTYPE_pd, 128-bit packed single/double real) as a memory address if it is the instruction's recorded addressing operand, otherwise as its register name.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand Packed-real operand supplying either the register to name or the address to format.
 * @param[in] OperandIndex Index of Operand within Instruction, used to test against the recorded source/destination addressing index.
 */
void OutputPackedReal(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Formats a packed-BCD operand (OPTYPE_pb, 10-byte 18-digit packed BCD) as a memory address if it is the instruction's recorded addressing operand, otherwise as its register name.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand Packed-BCD operand supplying either the register to name or the address to format.
 * @param[in] OperandIndex Index of Operand within Instruction, used to test against the recorded source/destination addressing index.
 */
void OutputPackedBCD(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Formats a scalar-real operand (OPTYPE_ss/sd/se: scalar single/double/extended real) as a memory address if it is the instruction's recorded addressing operand, otherwise as its register name.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand Scalar-real operand supplying either the register to name or the address to format.
 * @param[in] OperandIndex Index of Operand within Instruction, used to test against the recorded source/destination addressing index.
 */
void OutputScalarReal(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Formats an OPTYPE_sso/OPTYPE_sdo operand as a scalar real (via OutputScalarReal()) if it decoded to OPTYPE_FLOAT, otherwise as a general-purpose operand (via OutputGeneral()).
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand Operand whose decoded Type selects which formatter handles it.
 * @param[in] OperandIndex Index of Operand within Instruction, passed through to the chosen formatter.
 */
void OutputScalarGeneral(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Formats an OPTYPE_fev operand (FPU environment: 28 bytes in 32-bit modes, 14 bytes in 16-bit mode) as a memory address.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand FPU environment operand to format as an address.
 * @param[in] OperandIndex Index of Operand within Instruction, asserted to match the recorded source or destination addressing operand.
 */
void OutputFPUEnvironment(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Formats an OPTYPE_fst1 (FPU state: 108 bytes in 32-bit modes, 94 bytes in 16-bit real mode) or OPTYPE_fst2 (FPU/MMX/XMM/MXCSR state, 512 bytes) operand as a memory address.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand FPU/CPU extended-state operand to format as an address.
 * @param[in] OperandIndex Index of Operand within Instruction, asserted to match the recorded source or destination addressing operand.
 */
void OutputFPUState(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Formats an OPTYPE_cpu operand (pointer to a CPU state structure) as a memory address.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand CPU-state-pointer operand to format as an address.
 * @param[in] OperandIndex Index of Operand within Instruction, asserted to match the recorded source addressing operand.
 */
void OutputCPUState(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
    X86_INSTRUCTION* X86Instruction = &Instruction->X86;
...
/**
 * @brief Formats an OPTYPE_p operand (segment:offset pointer) as a memory address.
 * @param[in,out] Instruction Instruction being formatted; the formatted text is appended to its output string.
 * @param[in] Operand Segment:offset operand to format as an address.
 * @param[in] OperandIndex Index of Operand within Instruction, asserted to match the recorded source addressing operand.
 */
void OutputSegOffset(INSTRUCTION* Instruction, INSTRUCTION_OPERAND* Operand, U32 OperandIndex)
{
...
/**
 * @brief Intended to find the first function between StartAddress and EndAddress by matching a standard prologue and then analyzing the following instructions to verify it is a valid function; not yet implemented.
 ...
 * @return Always NULL; the body is `assert(0); // TODO` and has no implementation.
 */
...
/**
 * @brief Decodes (and, when requested, disassembles) one x86/x64 instruction at Address, resolving prefixes, its one/two/three-byte opcode and any opcode group or SSE extension against the tables in disasm_x86_tables.h, then delegating operand decoding to SetOperands() and classifying the result (branch targets, stack effects, anomalies, emulation needs).
 * @param[in,out] Instruction Instruction to decode into; Instruction->Address must already equal Address, and its X86 sub-structure and text output are populated by this call.
 * @param[in] Address Address of the first byte of the instruction to decode.
 * @param[in] Flags Decode flags, e.g. DISASM_DECODE to populate operand fields, DISASM_DISASSEMBLE to also format the text representation, DISASM_SUPPRESSERRORS to silence the diagnostic printf()s, and DISASM_ALIGNOUTPUT to pad operand columns.
 * @return TRUE if the instruction was successfully decoded; FALSE if an invalid opcode, prefix or other error was encountered (see the `abort` path).
 */
BOOL X86_GetInstruction(INSTRUCTION* Instruction, U8* Address, U32 Flags)
{
    ...
    while (TRUE)
    {
        ...
        if (IS_AMD64() && (Opcode >= REX_PREFIX_START && Opcode <= REX_PREFIX_END) &&
            X86_PREFIX((&X86_Opcodes_1[*Address])))
        {
            if (!Instruction->AnomalyOccurred)
            {
                if (!SuppressErrors)
        ...
        if (X86_PREFIX(X86Opcode))
        {
            ...
            case PREFIX_ADDRESS_SIZE:
                ...
                if (X86Instruction->HasAddressSizePrefix)
                    ...
                    X86Instruction->HasAddressSizePrefix = TRUE;
                switch (X86Instruction->AddressSize)
                {
                case 8:
                    X86Instruction->AddressSize = 4;
    ...
    if (IS_AMD64() && (Opcode >= REX_PREFIX_START && Opcode <= REX_PREFIX_END))
    {
        ...
        DISASM_OUTPUT(
            ("[0x%08I64X] REX prefix 0x%02X (prefix count %d, w=%d, r=%d, x=%d, b=%d)\n",
             ...
             Opcode,
             Instruction->PrefixCount,
             X86Instruction->rex.w,
             X86Instruction->rex.r,
             X86Instruction->rex.x,
             X86Instruction->rex.b)
        );
    ...
    if (Opcode == X86_TWO_BYTE_OPCODE)
    {
        ...
        else if (SSE_Prefix && !X86_INVALID(&X86_SSE[Opcode])) // SSEx instruction
        {
            ...
            {
                if (IS_AMD64() && X86Instruction->rex.w)
                    X86Instruction->OperandSize = 8;
                else
                    X86Instruction->OperandSize = 4;
            }
            ...
            case PREFIX_OPERAND_SIZE:
                X86Opcode = &X86_SSE[0x000 + Opcode];
                break;
            case PREFIX_REPNE:
                X86Opcode = &X86_SSE[0x100 + Opcode];
                break;
            case PREFIX_REP:
                X86Opcode = &X86_SSE[0x200 + Opcode];
                break;
            }
            ...
            else if (X86_EXTENDED_OPCODE(X86Opcode))
            {
                ...
                {
                case PREFIX_OPERAND_SIZE:
                    X86Opcode = &X86Opcode->Table[0x00 + OpcodeExtension];
                    break;
                case PREFIX_REPNE:
                    X86Opcode = &X86Opcode->Table[0x08 + OpcodeExtension];
                    break;
                case PREFIX_REP:
                    X86Opcode = &X86Opcode->Table[0x10 + OpcodeExtension];
                    break;
                }
...
HasSpecialExtension:
    if (SpecialExtension)
    {
        ...
        else if (X86Opcode->MnemonicFlags & ITYPE_EXT_SUFFIX)
        {
            ...
            if (!SetOperands(Instruction, Address, Flags & DISASM_SUPPRESSERRORS))
                goto abort;
        ...
        else if (X86Opcode->MnemonicFlags & ITYPE_EXT_64)
        {
            assert(X86Opcode->Table);
            if (IS_AMD64())
                X86Opcode = &X86Opcode->Table[1];
            else
                X86Opcode = &X86Opcode->Table[0];
    ...
    if (Disassemble)
    {
        if ((Flags & DISASM_SHOWFLAGS) && (X86Instruction->Opcode.Preconditions ||
                                           X86Instruction->Opcode.FlagsChanged || X86Instruction->Opcode.ResultsIfTrue))
        {
            APPENDPAD(124);
            ...
            if (X86Instruction->Opcode.FlagsChanged)
            {
                ...
                {
                    APPENDS("SET:{ ");
                    if (Result & FLAG_CF_SET)
                        APPENDS("C ");
                    if (Result & FLAG_DF_SET)
                        APPENDS("D ");
                    if (Result & FLAG_IF_SET)
                        APPENDS("I ");
                    APPENDB('}');
                }
                ...
                {
                    APPENDS("CLR:{ ");
                    if (Result & FLAG_SF_CLR)
                        APPENDS("S ");
                    if (Result & FLAG_ZF_CLR)
                        APPENDS("Z ");
                    if (Result & FLAG_AF_CLR)
                        APPENDS("A ");
                    if (Result & FLAG_CF_CLR)
                        APPENDS("C ");
                    if (Result & FLAG_DF_CLR)
                        APPENDS("D ");
                    if (Result & FLAG_IF_CLR)
                        APPENDS("I ");
                    if (Result & FLAG_OF_CLR)
                        APPENDS("O ");
                    ...
                    {
                        if (Result & FPU_C0_CLR)
                            APPENDS("FPU_C0 ");
                        if (Result & FPU_C1_CLR)
                            APPENDS("FPU_C1 ");
                        if (Result & FPU_C2_CLR)
                            APPENDS("FPU_C2 ");
                        if (Result & FPU_C3_CLR)
                            APPENDS("FPU_C3 ");
                    }
                    APPENDB('}');
                }
                ...
                if ((Result & FLAG_MOD_MASK) == FLAG_MOD_MASK)
                {
                    ...
                    {
                        if (Result & FLAG_OF_MOD)
                            APPENDS("O ");
                        if (Result & FLAG_SF_MOD)
                            APPENDS("S ");
                        if (Result & FLAG_ZF_MOD)
                            APPENDS("Z ");
                        if (Result & FLAG_AF_MOD)
                            APPENDS("A ");
                        if (Result & FLAG_PF_MOD)
                            APPENDS("P ");
                        if (Result & FLAG_CF_MOD)
                            APPENDS("C ");
                        if (Result & FLAG_DF_MOD)
                            APPENDS("D ");
                        if (Result & FLAG_IF_MOD)
                            APPENDS("I ");
                    }
                    ...
                    {
                        if (Result & FPU_C0_MOD)
                            APPENDS("FPU_C0 ");
                        if (Result & FPU_C1_MOD)
                            APPENDS("FPU_C1 ");
                        if (Result & FPU_C2_MOD)
                            APPENDS("FPU_C2 ");
                        if (Result & FPU_C3_MOD)
                            APPENDS("FPU_C3 ");
                    }
                    APPENDB('}');
                }
    ...
    if (Instruction->Groups & ITYPE_EXEC)
    {
        ...
        if (Operand1->Type == OPTYPE_OFFSET && Operand1->Length <= 2 && X86Instruction->Displacement < 0)
        {
        ...
        case ITYPE_BRANCH:
                      ...
                      (!X86Instruction->HasBaseRegister && X86Instruction->HasIndexRegister)))
            {
                ...
                {
                    if (Operand1->Length)
                        X86Instruction->Scale = (U8)Operand1->Length;
                    else
                        X86Instruction->Scale = X86Instruction->OperandSize;
                }
                assert(Operand1->Length <= 0xFF);
                      ...
                      (!X86Instruction->HasBaseRegister && X86Instruction->HasIndexRegister)))
            {
                DISASM_OUTPUT(
                    ("[0x%08I64X] Data reference (scale %d, size %d, displacement 0x%08I64x)\n",
                     ...
                     Operand->Length,
                     X86Instruction->Displacement)
                );
                if (!X86Instruction->Scale)
                {
                    assert(Operand->Length <= 0xFF);
                    if (Operand->Length)
                        X86Instruction->Scale = (U8)Operand->Length;
                    else
                        X86Instruction->Scale = X86Instruction->OperandSize;
                }
...
/**
 * @brief Decodes the resolved opcode's ModR/M byte (if any) and then each of its operands in turn, and, when disassembling, formats each one via its OPTYPE handler.
 * @param[in,out] Instruction Instruction whose ModR/M-derived fields, per-operand INSTRUCTION_OPERAND entries and text output are populated.
 * @param[in] Address Address of the first byte after the opcode (e.g. the first byte of the ModR/M byte or of an immediate value).
 * @param[in] Flags Decode flags forwarded from X86_GetInstruction(), e.g. DISASM_DECODE, DISASM_DISASSEMBLE and DISASM_SUPPRESSERRORS.
 * @return The address immediately following the last operand (e.g. the next operand or the start of the next instruction), or NULL on error (see the `abort` path).
 */
INTERNAL U8* SetOperands(INSTRUCTION* Instruction, U8* Address, U32 Flags)
{
    INSTRUCTION_OPERAND* Operand;
    ...
    for (OperandIndex = 0; OperandIndex < Instruction->OperandCount; OperandIndex++)
    {
        ...
        case OPTYPE_REG_xAX_BIG:
            ...
            {
            case 8:
                Operand->Register = AMD64_REG_RAX;
                break;
            case 4:
            ...
            if (Decode)
            {
                ...
                case 8:
                    if (OperandFlags & OP_SIGNED)
                        Operand->Value_S64 = (S64) * ((S64*)Address);
                    else
                        Operand->Value_U64 = (U64) * ((U64*)Address);
                    break;
                case 4:
                        ...
                        Instruction->Operands[0].Length == 8)
                    {
                        ...
                        default:
                            ...
                            if (OperandFlags & OP_SIGNED)
                                ...
                                Operand->Value_U64 = (U64) * ((U32*)Address);
                            break;
                        }
                    }
                    else
                ...
                case 2:
                    if (OperandFlags & OP_SIGNED)
                        Operand->Value_S64 = (S64) * ((S16*)Address);
                    else
                        Operand->Value_U64 = (U64) * ((U16*)Address);
                    break;
                case 1:
                    if (OperandFlags & OP_SIGNED)
                        Operand->Value_S64 = (S64) * ((S8*)Address);
                    else
                        Operand->Value_U64 = (U64) * ((U8*)Address);
                    break;
                default:
                    assert(0);
                    return NULL;
        ...
        case AMODE_J: // IP-relative jump offset
            ...
            if (Decode)
            {
                ...
                {
                case 8:
                    X86Instruction->Displacement = *((S64*)Address);
                    break;
                case 4:
        ...
        case AMODE_O: // word/dword offset
            ...
            case 8:
                if (Operand->Flags & OP_SIGNED)
                    ...
                    X86Instruction->Displacement = (S64) * ((U64*)Address);
                break;
            case 4:
                if (Operand->Flags & OP_SIGNED)
                    X86Instruction->Displacement = (S64) * ((S32*)Address);
                else
                    X86Instruction->Displacement = (S64) * ((U32*)Address);
                break;
            case 2:
                if (Operand->Flags & OP_SIGNED)
                    X86Instruction->Displacement = (S64) * ((S16*)Address);
                else
                    X86Instruction->Displacement = (S64) * ((U16*)Address);
                break;
            default:
                assert(0);
                return FALSE;
        ...
        case AMODE_A: // absolute address
            ...
            {
            case 6:
                X86Instruction->Segment = *((U16*)Address);
                INSTR_INC(2);
                X86Instruction->Displacement = (S64) * ((S32*)Address);
                INSTR_INC(4);
                break;
...
/**
 * @brief Decodes a 16-bit-addressing ModR/M byte: register-direct (mod=3), the absolute 16-bit displacement encoding (mod=0, rm=6), or one of the eight classic 8086 indirect addressing combinations (BX+SI, BX+DI, BP+SI, BP+DI, SI, DI, BP, BX) with an optional 8- or 16-bit displacement.
 * @param[in,out] Instruction Instruction whose X86 addressing fields (Displacement, BaseRegister, IndexRegister, HasBaseRegister/HasIndexRegister, Segment when BP is used) are set, and whose Length/Address are advanced past any displacement bytes consumed.
 * @param[in] Address Address one byte past the ModR/M byte, at the first potential displacement byte.
 * @param[in,out] Operand Operand being resolved; receives its Register and/or address-related Flags (OP_REG, OP_ADDRESS, OP_GLOBAL).
 * @param[in] OperandIndex Index of Operand within Instruction, used by the X86_SET_ADDR helper to record it as the source/destination addressing operand.
 * @param[in] SuppressErrors Accepted for signature parity with the other ModR/M decoders; not read in this function's body.
 * @return Address advanced past any displacement bytes consumed, or NULL if Operand->Length is not 1, 2 or 4 in the register-direct case.
 */
INTERNAL U8* SetModRM16(
    INSTRUCTION* Instruction,
    U8* Address,
...
// NOTE: Address points one byte after ModRM
/**
 * @brief Decodes a 32/64-bit-addressing ModR/M byte, delegating to SetModRM16() when the address size is 2: register-direct (mod=3), the disp32/RIP-relative encoding (mod=0, rm=5), SIB-indicated addressing (rm=4, via SetSIB()), or base-register-indirect addressing with an optional 8- or 32-bit displacement.
 * @param[in,out] Instruction Instruction whose X86 addressing fields (Displacement, BaseRegister, and, for the RIP-relative case, Relative/HasFullDisplacement/Segment) are set, and whose Length/Address are advanced past the bytes consumed.
 * @param[in] Address Address one byte past the ModR/M byte, at the first potential displacement byte (or the SIB byte when rm=4).
 * @param[in,out] Operand Operand being resolved; receives its Register, and, for the RIP-relative case, its precomputed TargetAddress, plus address-related Flags.
 * @param[in] OperandIndex Index of Operand within Instruction; used both to scan the instruction's later operands (to size a trailing immediate when computing a RIP-relative target) and by the X86_SET_ADDR/X86_SET_REG/X86_SET_SEG helpers.
 * @param[in] SuppressErrors Forwarded unchanged to SetModRM16() and SetSIB(); not read directly in this function's body.
 * @return Address advanced past the decoded addressing bytes, or NULL on an unrecognized operand length/address size, or when SetSIB() returns NULL.
 */
INTERNAL U8* SetModRM32(
    INSTRUCTION* Instruction,
    U8* Address,
...
/**
 * @brief Decodes the SIB (scale-index-base) byte, handling the base=5 special case (disp32, or disp8/disp32 relative to (E/R)BP or R13 depending on REX.B) and setting the index register and scale unless the SIB encodes "no index".
 * @param[in,out] Instruction Instruction whose X86 addressing fields (BaseRegister, IndexRegister, HasBaseRegister/HasIndexRegister, Scale, Displacement, and Segment when the base is (E)BP) are set, and whose Length/Address are advanced past the SIB byte and any displacement consumed.
 * @param[in] Address Address of the SIB byte.
 * @param[in,out] Operand Operand being resolved; receives its Register, has its TargetAddress cleared when an index register is present, and receives address-related Flags.
 * @param[in] OperandIndex Index of Operand within Instruction, used by the X86_SET_ADDR helper to record it as the source/destination addressing operand.
 * @param[in] SuppressErrors Accepted for signature parity with the other ModR/M decoders; not read in this function's body.
 * @return Address advanced past the SIB byte and any displacement it consumes, or NULL on an unrecognized address size.
 */
INTERNAL U8* SetSIB(
    INSTRUCTION* Instruction,
    U8* Address,
...
/**
 * @brief Adds the instruction's decoded displacement to Address, truncating the intermediate result to the instruction's operand size so a 16-bit or 32-bit IP wraps around the way the real addressing mode would (see SUPPORT_WRAPAROUND).
 * @param[in] Address Base 64-bit address (typically a virtual address already offset by Instruction->VirtualAddressDelta) to add the displacement to.
 * @param[in] Instruction Instruction supplying the displacement (X86.Displacement) and the operand size (X86.OperandSize) that determines the wraparound width.
 * @return Address plus the displacement, wrapped at the instruction's operand size when SUPPORT_WRAPAROUND is defined, or added without wraparound handling otherwise.
 */
INTERNAL U64 ApplyDisplacement(U64 Address, INSTRUCTION* Instruction)
{
    X86_INSTRUCTION* X86Instruction = &Instruction->X86;
...
/**
 * @brief Checks whether the decoded instruction's LOCK prefix is valid: whether the opcode (or, for a group opcode, its group and opcode extension) is listed as lockable, and whether the destination operand addresses memory.
 * @param[in] X86Instruction Decoded instruction consulted for its ModR/M fields (HasModRM, modrm.mod, HasDstAddressing) and, in the diagnostic message, its resolved mnemonic and owning instruction's address.
 * @param[in] Opcode Opcode byte used to index the one-byte (X86_LockPrefix_1) or two/three-byte (X86_LockPrefix_2) lock-eligibility table.
 * @param[in] OpcodeLength Selects which lock-eligibility table to consult: 1 for the one-byte table, 2 or 3 for the two/three-byte table.
 * @param[in] Group Opcode group number used with OpcodeExtension to index X86_LockPrefix_Groups when the table marks the opcode as group-dependent (GR).
 * @param[in] OpcodeExtension ModR/M reg-field opcode extension used with Group to index X86_LockPrefix_Groups.
 * @return TRUE if the opcode is marked lockable (directly, or via its group and extension) and its destination operand is a memory operand (ModR/M present, mod != 3, and recorded as the destination addressing operand); FALSE otherwise.
 */
INTERNAL BOOL
